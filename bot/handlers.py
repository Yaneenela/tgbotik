...
async def _process_payment(
    ...
    if renew_sub_id:
        sub = await db.get_subscription(renew_sub_id)
        ...
        if sub.get("expired_at"):
            try:
                ...
                remaining_days = 0
        email = sub.get("email") or f"tg_{tg_id}"
        try:
            await xui.update_client_expiry(sub["uuid"], email, plan.days + remaining_days, device_count)
        except Exception as e:
            logger.error(f"3x-UI extend error: {e}")
            await bot.send_message(tg_id, f"Ошибка продления: {e}")
            return False
        await db.extend_expiry(sub["id"], plan.days)
        await db.update_sub_device_count(sub["id"], device_count)
        client_uuid = sub["uuid"]
        is_renewal = True
    else:
        try:
            email = f"tg{tg_id}_{uuid_lib.uuid4().hex[:8]}"
...
def create_router(cfg: Config, db: Database, xui: XUIManager):
    ...
    async def cmd_start(message: Message):
        ...
        await message.answer_photo(
            ...
            caption=(
                ...
                f"Наш сервис открывает доступ ко всем ресурсам в интернете "
                f"и бережно защищает вашу конфиденциальность."
            ),
    ...
    async def cb_confirm_device(callback: CallbackQuery, state: FSMContext):
        ...
        if idx is None or device_count is None:
            await _nav(callback, "Ошибка: начните заказ заново.", back_button())
            await state.clear()
            return
        plan = cfg.plans[idx]
        total = calc_total_price(plan, device_count)
    ...
    async def cb_pay_crypto(callback: CallbackQuery, state: FSMContext, bot: Bot):
        ...
        if not crypto:
            await _nav(callback, "Оплата через CryptoBot недоступна.", back_button())
            return
        ...
        try:
            ...
            usdt_rate = 0
        if usdt_rate <= 0:
            await _nav(callback, "Ошибка получения курса. Попробуйте позже.", back_button())
            return
        ...
        if not invoice:
            await _nav(callback, "Ошибка создания счёта. Попробуйте позже.", back_button())
            return
    ...
    async def cb_cancel_pay(callback: CallbackQuery):
        await callback.answer(
            "Отмена больше не нужна — если счёт не оплатить, он закроется сам.",
            show_alert=True,
        )
    ...
    async def cb_check_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
        ...
        idx = int(idx_str)
        if idx >= len(cfg.plans):
            await callback.answer("Тариф изменился. Начните выбор заново.", show_alert=True)
            return
        plan = cfg.plans[idx]
        tg_id = callback.from_user.id
            ...
            if tx:
                if tx.status == "CONFIRMED":
                    try:
                        ...
                        paid_amount = 0.0
                    if paid_amount and abs(paid_amount - own["amount"]) > 1:
                        await db.update_transaction(pay_id, "failed")
                        await callback.answer(
                            "Сумма платежа не совпадает с суммой заказа. Напишите в поддержку.",
                            show_alert=True,
                        )
                        return
                    paid = True
        ...
        if paid:
            ...
            ok = False
            try:
                ok = await _process_payment(cfg, db, xui, bot, tg_id, plan, pay_id, device_count, renew_sub_id)
            except Exception as e:
                logger.error(f"Payment activation error: {e}")
            if not ok:
                await db.update_transaction(pay_id, "pending")
                await callback.answer(
                    "Не удалось активировать подписку. Попробуйте оплату ещё раз позже или используйте «Я оплатил».",
                    show_alert=True,
                )
                return
    ...
    async def cb_renew_sub(callback: CallbackQuery, state: FSMContext):
    ...
    async def cb_my_subs(callback: CallbackQuery):
        ...
        subs = await db.get_user_all_subscriptions(user["id"])
        if not subs:
            await _nav(callback, "У вас нет подписок.", plans_keyboard(cfg.plans, "plan"))
            return
    ...
    async def cb_edit_devices(callback: CallbackQuery):
        ...
        if not subs:
            await _nav(callback, "Нет активных подписок.", back_button())
            return
        builder = InlineKeyboardBuilder()
        for s in subs:
    ...
    async def cb_upgrade_dev_confirm(callback: CallbackQuery, bot: Bot):
        ...
        if new_count < current:
            ...
            try:
                ...
                await xui.update_client_expiry(sub["uuid"], sub.get("email") or f"tg_{tg_id}", remaining_days, new_count)
            except Exception as e:
    ...
    async def cb_upgrade_pay_check(callback: CallbackQuery, bot: Bot):
        ...
        try:
            ...
            await db.update_transaction(str(invoice_id), "pending")
            await _nav(callback, f"Ошибка 3x-UI: {e}", back_button())
            return
    ...
    async def cb_upgrade_platega_pay_check(callback: CallbackQuery, bot: Bot):
        ...
        try:
            ...
            paid_amount = 0.0
        if paid_amount and abs(paid_amount - own["amount"]) > 1:
            await db.update_transaction(transaction_id, "failed")
            await callback.answer(
                "Сумма платежа не совпадает с суммой заказа. Напишите в поддержку.",
                show_alert=True,
            )
        ...
        try:
            ...
            await db.update_transaction(transaction_id, "pending")
            await _nav(callback, f"Ошибка 3x-UI: {e}", back_button())
            return
    ...
    async def msg_admin_broadcast(message: Message, bot: Bot, state: FSMContext):
        ...
        for u in users:
            try:
                await bot.send_message(u["telegram_id"], text, parse_mode=None)
                sent += 1
            except Exception:
                pass
    ...
    async def cb_admin_grant_plan(callback: CallbackQuery, state: FSMContext):
        ...
        if tg_id is None:
            await _nav(callback, "Ошибка: начните выдачу заново.", admin_menu())
            await state.clear()
            return
        builder = InlineKeyboardBuilder()
        for i in range(1, 11):
    ...
    async def cb_admin_sub_select(callback: CallbackQuery):
        ...
        sub = await db.get_subscription(sub_id)
        if not sub:
            await _nav(callback, "Подписка не найдена.", admin_menu())
            return
    ...
    async def cb_admin_sub_delete_confirm(callback: CallbackQuery):
        ...
        try:
            ...
            await _nav(callback, f"Ошибка 3x-UI: {e}", admin_menu())
            return
